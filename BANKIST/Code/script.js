...
const incomes = acc.movements.filter(mov=>mov>0).reduce((acc,mov,i,array)=>acc+mov,0);
labelSumIn.textContent = `${incomes}€`

const out = acc.movements.filter(mov=>mov<0).reduce((acc,mov,i,array)=>acc+mov,0);
...
if(amount>0 && currentAccount.movements.some(mov=> mov>=(10/100*amount))){
 // ADD movement
 currentAccount.movements.push(amount);
 // Update UI
 updateUI(currentAccount)
